...
<div class="max-w-4xl space-y-6">
    ...
    {{-- Form Card --}}
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden" x-data="{ role: '{{ old('role', $user->role) }}' }">
        <div class="px-6 py-4 border-b border-gray-100">
            <h1 class="text-lg font-bold text-gray-900">Edit User — {{ $user->name }}</h1>
        </div>
        ...
        <div class="p-6">
            ...
            <form method="POST" action="{{ route('admin.users.update', $user) }}">
                        ...
                        @foreach($menuGroups as $group)
                            ...
                            <div class="border border-gray-200 rounded-xl p-4 hover:border-emerald-200 transition-colors">
                                ...
                                @foreach($group['items'] as $item)
                                    @php
                                        $checkedPerms = $userPermissions[$item['slug']] ?? [];
                                        $hasAny = count($checkedPerms) > 0;
                                    @endphp
                                    <div class="ml-4 mb-3">
                                        <div class="flex items-center mb-2">
                                            <input type="checkbox" id="perm_{{ $item['slug'] }}" {{ $hasAny ? 'checked' : '' }}
                                                   class="perm-check w-4 h-4 text-emerald-600 border-gray-300 rounded focus:ring-emerald-500">
                                        ...
                                        <div class="ml-6 flex flex-wrap gap-3">
                                            @foreach($item['permissions'] as $perm)
                                                @php $isChecked = in_array($perm, $checkedPerms); @endphp
                ...
                <div class="flex items-center justify-end space-x-3 mt-8 pt-6 border-t border-gray-200">
                    ...
                    <button type="submit" class="px-5 py-2.5 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-all duration-200 text-sm font-medium">Update User</button>
                </div>
            </form>
        </div>
    </div>
