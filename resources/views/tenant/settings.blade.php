                    ...
                    <div class="p-6">
                        ...
                        <div x-show="tab === 'delivery'" x-transition>
                            ...
                            <form method="POST" action="{{ route('settings.business.update') }}" enctype="multipart/form-data">
                                ...
                                <div class="mb-6" x-data="{ logoPreview: '{{ $businessSetting->logo_path ? $businessSetting->getLogoUrl() : '' }}', fileName: '' }">
                                    @if($businessSetting->logo_path)
                                        <div class="mb-4">
                                            <p class="text-xs text-gray-400 mb-2">বর্তমান লোগো:</p>
                                            <img src="{{ $businessSetting->getLogoUrl() }}" alt="Current Logo" class="h-20 rounded-xl object-contain border border-gray-200">
                                        </div>
                                    @endif
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 mb-1">@lang('settings.upload_logo')</label>
                                        <div class="relative">
                                            <label for="logoUpload" class="flex items-center gap-3 w-full px-4 py-3 border-2 border-dashed border-gray-300 rounded-xl hover:border-purple-400 hover:bg-purple-50/50 transition cursor-pointer">
...
function settingsApp() {
    return {
        tab: '{{ session("active_tab", "business") }}',
