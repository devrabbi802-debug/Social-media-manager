                                        ...
                                        <td class="px-4 py-3">
                                            <div class="flex flex-wrap gap-1 items-start">
                                                ...
                                                <div class="relative group" x-data>
                                                    ...
                                                    <label class="absolute top-0 right-0 flex items-center cursor-pointer opacity-0 group-hover:opacity-100 transition-opacity">
                                                        <input type="checkbox" name="delete_variant_images[{{ $variant->id }}][]" value="{{ $img->id }}" class="w-3 h-3 text-red-600 rounded">
                                                    </label>
                                                </div>
                                                @endforeach
                                                <label class="cursor-pointer inline-flex items-center px-2 py-1 bg-purple-100 text-purple-700 rounded-lg text-xs font-medium hover:bg-purple-200 transition">
                                                    ...
                                                    <input type="file" name="variant_images[{{ $variant->id }}][]" multiple accept="image/*" class="hidden"
                                                           onchange="previewVariantEditImages(this)">
                                                </label>
                                            </div>
                                        </td>
                                    </tr>
...
function previewVariantEditImages(input) {
    const td = input.closest('td');
    const container = td.querySelector('.flex');
    Array.from(input.files).forEach(file => {
        const reader = new FileReader();
        reader.onload = (ev) => {
            const div = document.createElement('div');
            div.className = 'relative w-10 h-10';
            div.innerHTML = `<img src="${ev.target.result}" class="w-10 h-10 object-cover rounded-lg">`;
