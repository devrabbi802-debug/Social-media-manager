                ...
                <table class="w-full">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">@lang('attributes.is_variant')</th>
                            <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">@lang('attributes.is_filterable')</th>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @foreach($attributes as $attribute)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 text-sm text-gray-500">{{ $attribute->data_type }}</td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    @if($attribute->is_filterable)
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">@lang('attributes.yes')</span>
                                    @else
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800">@lang('attributes.no')</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4 text-right text-sm">
                                    <div class="flex items-center justify-end space-x-2">
                                        <a href="{{ route('inventory.attributes.edit', $attribute) }}" class="text-blue-600 hover:text-blue-800 p-1">@lang('common.edit')</a>
                                        <form action="{{ route('inventory.attributes.destroy', $attribute) }}" method="POST" onsubmit="return confirm('{{ __('attributes.delete_confirm') }}')">
                                            @csrf
                                            @method('DELETE')
