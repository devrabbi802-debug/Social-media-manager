...
@section('content')
@php
    $existingValues = $attribute->attributeValues->map(fn($v) => [
        'id' => $v->id,
        'value' => $v->value,
            ...
            <div class="bg-white rounded-2xl p-6 shadow-sm space-y-4">
                <div id="category-field">
                    <x-searchable-select
                        ...
                        :selected="old('category_id', $attribute->category_id)"
                ...
                <div>
                    ...
                    <select name="data_type" id="data-type-select" class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent">
                        <option value="text" {{ old('data_type', $attribute->data_type) === 'text' ? 'selected' : '' }}>Text</option>
                        <option value="textarea" {{ old('data_type', $attribute->data_type) === 'textarea' ? 'selected' : '' }}>Textarea</option>
                        <option value="number" {{ old('data_type', $attribute->data_type) === 'number' ? 'selected' : '' }}>Number</option>
                        <option value="select" {{ old('data_type', $attribute->data_type) === 'select' ? 'selected' : '' }}>Select</option>
                        <option value="multiselect" {{ old('data_type', $attribute->data_type) === 'multiselect' ? 'selected' : '' }}>Multi Select</option>
                        <option value="boolean" {{ old('data_type', $attribute->data_type) === 'boolean' ? 'selected' : '' }}>Boolean (Yes/No)</option>
                        <option value="date" {{ old('data_type', $attribute->data_type) === 'date' ? 'selected' : '' }}>Date</option>
                    </select>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="flex items-center space-x-3">
                            <input type="checkbox" name="is_variant" value="1" {{ old('is_variant', $attribute->is_variant) ? 'checked' : '' }} class="w-5 h-5 text-purple-600 border-gray-300 rounded focus:ring-purple-500">
                            <span class="text-sm text-gray-700">@lang('attributes.mark_variant')</span>
                        </label>
                    </div>
                    <div>
                        <label class="flex items-center space-x-3">
                            <input type="checkbox" name="is_filterable" value="1" {{ old('is_filterable', $attribute->is_filterable) ? 'checked' : '' }} class="w-5 h-5 text-purple-600 border-gray-300 rounded focus:ring-purple-500">
                ...
                <div>
                    <label class="flex items-center space-x-3">
                        <input type="checkbox" name="is_global" value="1" {{ old('is_global', $attribute->is_global) ? 'checked' : '' }} class="w-5 h-5 text-purple-600 border-gray-300 rounded focus:ring-purple-500">
                        <span class="text-sm text-gray-700">@lang('attributes.is_global')</span>
                    </label>
                </div>

                <!-- Attribute Values (for select/multiselect) -->
                <div id="values-section" class="{{ in_array($attribute->data_type, ['select', 'multiselect']) ? '' : 'hidden' }}" x-data="{
                    values: @js($existingValues),
                    newValue: '',
                    newSwatch: '#000000',
                    showSwatch: false,
                    editingColor: null,
