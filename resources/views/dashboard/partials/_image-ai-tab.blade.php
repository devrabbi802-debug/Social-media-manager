...
<div class="bg-white rounded-2xl p-6 shadow-sm">
    <div class="flex items-center mb-6">
        ...
        <div>
            <h2 class="text-lg font-bold text-gray-900">নতুন Image AI Key যোগ করুন</h2>
            <p class="text-sm text-gray-500">Google Gemini API Key যোগ করুন</p>
        </div>
    </div>
    ...
    <form action="{{ route('ai.setup.store') }}" method="POST">
        ...
        <div class="mb-4">
            <label for="image_api_key" class="block text-sm font-medium text-gray-700 mb-2">Gemini API Key *</label>
            <input
                type="password"
                id="image_api_key"
                name="api_key"
...
@if($imageKeys->isNotEmpty())
    <div class="bg-white rounded-2xl p-6 shadow-sm">
        <div class="flex items-center mb-6">
            ...
            <div>
                <h2 class="text-lg font-bold text-gray-900">Image AI Keys ({{ $imageKeys->count() }})</h2>
                <p class="text-sm text-gray-500">Google Gemini API Key — ইমেজ তৈরির জন্য</p>
            </div>
        </div>

        <div class="space-y-3">
            @foreach($imageKeys as $index => $setting)
                <div class="border border-gray-200 rounded-xl p-4 {{ $setting->is_active ? 'bg-white' : 'bg-gray-50 opacity-60' }}">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-4">
                            <div class="w-10 h-10 {{ $setting->is_active ? 'bg-blue-100' : 'bg-gray-200' }} rounded-full flex items-center justify-center flex-shrink-0">
                                <span class="text-sm font-bold {{ $setting->is_active ? 'text-blue-600' : 'text-gray-500' }}">#{{ $index + 1 }}</span>
                            </div>
                            <div>
