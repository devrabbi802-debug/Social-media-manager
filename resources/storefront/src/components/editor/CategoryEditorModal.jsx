...
export default function CategoryEditorModal({ sectionData, onClose, onSaved }) {
  const [allCategories, setAllCategories] = useState([]);
  const [selected, setSelected] = useState(() => {
    if (sectionData?.categories?.length > 0) {
      return sectionData.categories.map((c, i) => ({ ...c, _key: Date.now() + i }));
    }
    return [];
  });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await api.put('/editor/sections/categories', { categories: payload });
      window.__editor_categories = res.categories;
      if (onSaved) onSaved(res.categories);
      if (sectionData?.onCategoriesSaved) sectionData.onCategoriesSaved(res.categories);
      onClose();
    } catch (err) {
  ...
  return (
    <SectionEditorModal title="Featured Categories (Grid)" onClose={onClose}>
      <div className="space-y-6">
        <p className="text-sm text-gray-500">
          Select up to 5 categories for the grid display. Reorder with arrows, upload custom images.
        </p>
            ...
            {allCategories.map((cat) => {
              ...
              return (
                <button
                  ...
                  disabled={!sel && isMaxed}
                  className={`relative flex items-center gap-3 p-3 rounded-lg border text-left transition ${
                    sel
                      ? 'border-blue-500 bg-blue-50 ring-1 ring-blue-500'
                      : 'border-gray-200 hover:border-gray-300 hover:bg-gray-50'
                  } ${!sel && isMaxed ? 'opacity-40 cursor-not-allowed' : 'cursor-pointer'}`}
                >
        ...
        {selected.length > 0 && (
          <div className="space-y-3">
            <h3 className="text-sm font-semibold text-gray-700">Selected ({selected.length}/5)</h3>
            {selected.map((cat, index) => (
              <div key={cat._key} className="border border-purple-200 bg-purple-50/30 rounded-lg p-4">
